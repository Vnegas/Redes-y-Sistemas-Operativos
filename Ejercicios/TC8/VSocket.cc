...
int VSocket::DoAccept(){
  ...
  if (this->IPv6) {
    ...
    st = accept(this->idSocket, (struct sockaddr* ) &host6, sizeof(host6));
  } else {
    struct sockaddr_in  host4;
    memset( (char *) &host4, 0, sizeof( host4 ) );
    host4.sin_family = AF_INET;
    ...
    st = accept(this->idSocket, (struct sockaddr* ) &host4, sizeof(host4));
  }
...
size_t VSocket::sendTo( const void * buffer, size_t size, void * addr ) {
  ...
  if (this->IPv6) {
    st = sendto(this->idSocket, buffer, size, 0, (sockaddr *) addr, sizeof(*(sockaddr_in6 *) addr));
  } else {
    st = sendto(this->idSocket, buffer, size, 0, (sockaddr *) addr, sizeof(*(sockaddr *) addr));
  }
  
  if ( -1 == st ) {
    throw( std::runtime_error( "VSocket::SendTo, send" ) );
  }
...
size_t VSocket::recvFrom( void * buffer, size_t size, void * addr ) {
  ...
  if (this->IPv6) {
    socklen_t len = sizeof(*(sockaddr_in6 *) addr);
    st = recvfrom(this->idSocket, buffer, size, 0, (sockaddr *) addr, &len);
  } else {
  ...
  if ( -1 == st ) {
    throw( std::runtime_error( "VSocket::RecvFrom, recv" ) );
  }
