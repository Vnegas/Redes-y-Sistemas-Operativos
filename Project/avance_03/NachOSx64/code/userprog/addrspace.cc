...
AddrSpace::AddrSpace(AddrSpace *addr) {
    ...
    for (; i < size; i++) {
        pageTable[i].virtualPage = i;	// for now, virtual page # = phys page #
        pageTable[i].physicalPage = addr->pageTable[i].physicalPage; // the location is the position within the bitmap
        pageTable[i].valid = true;
        pageTable[i].use = false;
        pageTable[i].dirty = false;
        pageTable[i].readOnly = false;  // if the code segment was entirely on 
    ...
    for (; i < this->numPages; i++) {
        pageTable[i].virtualPage = i;	// for now, virtual page # = phys page #
        pageTable[i].physicalPage = pageTableMap->Find(); // the location is the position within the bitmap
        pageTable[i].valid = true;
        pageTable[i].use = false;
        pageTable[i].dirty = false;
        pageTable[i].readOnly = false;  // if the code segment was entirely on 
...
AddrSpace::~AddrSpace()
{
    for (unsigned int page = 0; page < this->numPages; page++) {
        pageTableMap->Clear(this->pageTable[page].physicalPage);
    }
   delete pageTable;
