...
Lock::Lock(const char* debugName) {
    ...
    this->value = 1;
    this->thread = NULL;
    this->semaphore = new Semaphore(debugName, value);
}
...
void Condition::Wait( Lock * conditionLock ) {
    if(conditionLock->isHeldByCurrentThread()) {
        Semaphore* semaphore = new Semaphore("sem", 0);
        this->queue->Append(semaphore);
        conditionLock->Release();
        semaphore->P();
        conditionLock->Acquire();
        delete semaphore;
    } else {
        printf(" Error: Condition::Wait( Lock * conditionLock )");
        Exit(1);
    }
}
...
void Condition::Signal( Lock * conditionLock ) {
    if(conditionLock->isHeldByCurrentThread()) {
        ...
        printf(" Error: Condition::Signal( Lock * conditionLock )");
        Exit(1);    
    }
}
