...
void SSLSocket::Init( bool serverContext ) {
   ...
   if ( nullptr == ssl ) {
      throw std::runtime_error( "SSLSocket::Init( bool ): ssl = nullptr" );
   }
...
void SSLSocket::InitContext( bool serverContext ) {
   ...
   if ( nullptr == method ) {
      throw std::runtime_error( "SSLSocket::InitContext( bool ): method = nullptr" );
   }

   SSL_CTX * context = SSL_CTX_new( method );
   if ( nullptr == context ) {
      throw std::runtime_error( "SSLSocket::InitContext( bool ): context = nullptr" );
   }
...
int SSLSocket::Connect( const char * hostName, int port ) {
   ...
   if ( st == -1 ) {
      throw std::runtime_error( "SSLSocket::Connect( const char * hostName, int port ): DoConnect()" );
   }
   st = SSL_set_fd((SSL *) this->SSLStruct ,this->idSocket);
   if ( st == -1 ) {
      throw std::runtime_error( "SSLSocket::Connect( const char * hostName, int port ): SSL_set_fd()" );
   }
   st = SSL_connect((SSL *) this->SSLStruct);
   if ( st == -1 ) {
      throw std::runtime_error( "SSLSocket::Connect( const char * hostName, int port ): SSL_connect()" );
   }
   return st;
...
int SSLSocket::Connect( const char * host, const char * service ) {
   ...
   if ( st == -1 ) {
      throw std::runtime_error( "SSLSocket::Connect( const char * hostName, const char * service ): DoConnect()" );
   }
   st = SSL_set_fd((SSL *) this->SSLStruct ,this->idSocket);
   if ( st == -1 ) {
      throw std::runtime_error( "SSLSocket::Connect( const char * hostName, const char * service ): SSL_set_fd()" );
   }
   st = SSL_connect((SSL *) this->SSLStruct);
   if ( st == -1 ) {
      throw std::runtime_error( "SSLSocket::Connect( const char * hostName, const char * service ): SSL_connect()" );
   }
   return st;
...
size_t SSLSocket::Write( const char * string ) {
   int st =  SSL_write((SSL *) this->SSLStruct, (const void *) string, strlen(string));
